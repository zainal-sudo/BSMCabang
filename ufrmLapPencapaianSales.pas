...
type
  TfrmLapPencapaianSales = class(TForm)
...
var
  ...
  frmLapPencapaianSales: TfrmLapPencapaianSales;
...
procedure TfrmLapPencapaianSales.FormDblClick(Sender: TObject);
begin
  WindowState := wsMaximized;
end;

procedure TfrmLapPencapaianSales.btnExitClick(Sender: TObject);
begin
      Release;
end;

procedure TfrmLapPencapaianSales.refreshdata;
begin
...
procedure TfrmLapPencapaianSales.sbNewClick(Sender: TObject);
begin
   refreshdata;
//   startdate.SetFocus;
...
procedure TfrmLapPencapaianSales.FormShow(Sender: TObject);
begin
  flagedit := False;
//  startdate.DateTime := Date;
...
procedure TfrmLapPencapaianSales.FormKeyPress(Sender: TObject; var Key: Char);
begin
    if Key = #13 then
...
procedure TfrmLapPencapaianSales.sbPrintClick(Sender: TObject);
begin
  refreshdata;
end;

procedure TfrmLapPencapaianSales.loaddata;
var
  skolom,s: string ;
  afilter : string ;
  i:integer;
begin
           ...
           s:= 'select Bulan,Tahun,sls_kode,Salesman,Riil,Target,Ratio,RiilThnLalu, '
              + ' cast(Riil/RiilThnLalu*100 as decimal(5,2)) Grow from ( '
              + ' SELECT Bulan,Tahun,sls_kode,sls_nama Salesman,Riil,Target,Ratio, '
              + ' (select Riil from growth2 z where a.bulan=z.bulan and z.tahun='+inttostr(strtoint(edttahun.Text)-1)+' and a.sls_kode=z.sls_kode) RiilThnLalu '
              + ' FROM growth2 a '
              + ' where tahun='+edttahun.Text+') final';
  ds3.Close;
        ...
        Skolom :='Bulan,Tahun,Salesman,Riil,Target,Ratio,RiilThnLalu,Grow';
...
//  hitung;
          TcxDBPivotHelper(cxPivot).LoadFromCDS(ds3);
           ...
           SetPivotData(['Riil','Ratio','RiilThnLalu','Grow']);

end;

procedure TfrmLapPencapaianSales.btnTampilClick(Sender: TObject);
begin
    loaddata;

end;

procedure TfrmLapPencapaianSales.cxPageControl1Click(Sender: TObject);
begin
...
procedure TfrmLapPencapaianSales.TeSpeedButton1Click(Sender: TObject);
begin
...
procedure TfrmLapPencapaianSales.dttanggalChange(Sender: TObject);
begin
  enddate.DateTime := startdate.DateTime;
end;

function TfrmLapPencapaianSales.GetPivotChartLink: TcxPivotGridChartConnection;
begin
...
procedure TfrmLapPencapaianSales.TeSpeedButton2Click(Sender: TObject);
begin
//  IF PageControl1.Pages[1].Visible  then
...
procedure TfrmLapPencapaianSales.SetPivotRow(ColumnSets: Array Of String);
begin
  TcxDBPivotHelper(cxPivot).SetRowColumns(ColumnSets);
end;

procedure TfrmLapPencapaianSales.SetPivotColumns(ColumnSets: Array Of String);
begin

  TcxDBPivotHelper(cxPivot).SetColColumns(ColumnSets);
end;

procedure TfrmLapPencapaianSales.SetPivotData(ColumnSets: Array Of String);
begin
...
procedure TfrmLapPencapaianSales.cxButton1Click(Sender: TObject);
begin
...
procedure TfrmLapPencapaianSales.cxButton3Click(Sender: TObject);
  var
  s: string ;
  ftsreport : TTSReport;
begin
